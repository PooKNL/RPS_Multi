...
describe('<GameItem />', () => {
  const container = shallow(<GameItem { ...recipe } />)
  ...
  describe('when it is vegan, it', () => {
    const container = shallow(<GameItem { ...recipe } vegan={ true } />)
  ...
  describe('when it is pescatarian, it', () => {
    const container = shallow(<GameItem { ...recipe } pescatarian={ true } vegetarian={ false } />)
    it('shows a 🐟', () => {
      expect(container.find('ul > li')).to.have.text('🐟')
    })
  })
